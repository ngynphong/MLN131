...
// Component Page (Từng trang tạp chí)
// const Page = forwardRef(({ children, pageNumber, totalPages }, ref) => {
//   return (
//     <div ref={ref} className="bg-white text-gray-800 shadow-lg overflow-hidden flex flex-col">
//       <div className="flex-grow p-6 md:p-10 flex flex-col">
//         {children}
//       </div>
//       <div className="text-center p-2 text-sm text-gray-500 border-t border-gray-200">
//         {pageNumber > 0 && pageNumber < totalPages - 1 && `Trang ${pageNumber}`}
//       </div>
//     </div>
//   );
// });
...
const Magazine = () => {
  ...
  const pages = magazineContent.pages.map((pageData, index) => {
    switch (pageData.type) {
      case 'cover':
        return (
          <div key={index} className="w-full h-full flex flex-col justify-center items-center text-center text-white p-4 sm:p-6 md:p-8"
            ...
            <div className="relative z-10 flex flex-col items-center px-4">
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black tracking-widest text-white" style={{ textShadow: '4px 4px 8px rgba(0,0,0,0.9)' }}>{pageData.title}</h2>
              <p className="mt-4 text-sm sm:text-base md:text-lg lg:text-xl max-w-md text-white" style={{ textShadow: '3px 3px 6px rgba(0,0,0,0.8)' }}>{pageData.subtitle}</p>
            </div>
          </div>
        );
      case 'page':
        return (
          <div key={index} className="w-full h-full p-4 sm:p-6 md:p-8 bg-[#F5F5DC] flex flex-col">
            <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-4 border-b-4 border-[#8B0000] pb-2">{pageData.title}</h3>
            <div className="space-y-3 text-sm sm:text-base text-gray-800 leading-relaxed">
              {pageData?.content?.map((item, i) => (
                <p key={i} dangerouslySetInnerHTML={{ __html: typeof item === 'string' ? item : item.text }} />
              ))}
            </div>
          </div>
        );
      case 'spread':
        return (
          <div key={index} className="w-full h-full grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 p-4 sm:p-6 md:p-8 bg-[#F5F5DC]">
            {/* Left Side */}
            <div className="flex flex-col justify-center">
              {pageData?.left?.imageUrl ? (
                <div className='hidden sm:block'>
                  <img src={pageData.left.imageUrl} alt={pageData.left.caption || 'Image'} className="rounded-lg shadow-xl w-full h-auto object-cover max-h-60 sm:max-h-80 border-4 border-[#8B0000]" />
                  {pageData.left.caption && <p className="text-xs sm:text-sm text-gray-700 mt-2 italic text-center font-medium">{pageData.left.caption}</p>}
                </div>
              ) : (
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{pageData?.left?.title}</h3>
                  <p className="text-base sm:text-lg text-[#8B0000] font-semibold mt-1 mb-3">{pageData?.left?.subtitle}</p>
                  <div className="space-y-3 text-sm sm:text-base text-gray-800 leading-relaxed">
                    {pageData?.left?.isList ? (
                      <ul className="list-disc list-inside space-y-2">
                        {pageData.left.content?.map((item, i) => (
                          <li key={i} dangerouslySetInnerHTML={{ __html: typeof item === 'string' ? item : (item as {text: string}).text }} />
                        ))}
                      </ul>
                    ) : (
                      pageData?.left?.content?.map((item, i) => <p key={i} dangerouslySetInnerHTML={{ __html: typeof item === 'string' ? item : (item as {text: string}).text }} />)
                    )}
                  </div>
                </div>
              )}
            ...
            <div className="flex flex-col justify-center">
              {pageData?.right?.imageUrl ? (
                <div className='hidden sm:block'>
                  <img src={pageData.right.imageUrl} alt={pageData.right.caption || 'Image'} className="rounded-lg shadow-xl w-full h-auto object-cover max-h-60 sm:max-h-80 border-4 border-[#8B0000]" />
                  {pageData.right.caption && <p className="text-xs sm:text-sm text-gray-700 mt-2 italic text-center font-medium">{pageData.right.caption}</p>}
                </div>
              ) : (
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{pageData?.right?.title}</h3>
                  <p className="text-base sm:text-lg text-[#8B0000] font-semibold mt-1 mb-3">{pageData?.right?.subtitle}</p>
                  <div className="space-y-3 text-sm sm:text-base text-gray-800 leading-relaxed">
                    {pageData?.right?.isList ? (
                      <ul className="list-disc list-inside space-y-2">
                        {pageData.right.content.map((item, i) => (
                          <li key={i} dangerouslySetInnerHTML={{ __html: typeof item === 'string' ? item : (item as {text: string}).text }} />
                        ))}
                      </ul>
                    ) : (
                      pageData?.right?.content?.map((item, i) => <p key={i} dangerouslySetInnerHTML={{ __html: typeof item === 'string' ? item : (item as {text: string}).text }} />)
                    )}
                  </div>
                </div>
              )}
      ...
      case 'back-cover':
        return (
          <div key={index} className="w-full h-full flex flex-col justify-center items-center text-center text-white p-4 sm:p-6 md:p-8" style={{ background: 'linear-gradient(135deg, #8B0000 0%, #FFD700 100%)' }}>
            <blockquote className="text-lg sm:text-xl md:text-2xl italic font-serif max-w-xl text-white px-4" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.7)' }}>"{pageData.quote}"</blockquote>
            <p className="mt-4 text-lg sm:text-xl font-bold text-white" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.7)' }}>- {pageData.author} -</p>
            <p className="mt-12 text-sm text-white opacity-80">{pageData.credits}</p>
          </div>
        );
      default:
        return null;
  ...
  const handleMagazineClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // If click on left side, go previous; right side, go next
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-4">
      <div
        className="w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl h-[60vh] sm:h-[70vh] md:h-[75vh] lg:h-[80vh] aspect-[4/5] bg-white rounded-lg shadow-2xl overflow-hidden relative magazine-container cursor-pointer"
        onClick={handleMagazineClick}
      >
        <div className={`w-full h-full magazine-pages ${currentPage > 0 ? 'page-flip' : ''}`}>
          {/* Render current and next page for flip effect, simplified for demonstration */}
          <div className="absolute w-full h-full backface-hidden page-shadow">
            {pages[currentPage]}
          </div>
        ...
        {/* Click indicator */}
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-gray-500 text-sm opacity-0 sm:opacity-70">
          Click để lật trang
        </div>
      </div>
      ...
      {/* <div className="flex justify-between items-center w-full max-w-6xl mt-6 px-4">
        <button
          ...
          className="px-6 py-3 text-white font-bold rounded-lg shadow-md hover:opacity-90 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-300 flex items-center"
        ...
        <div className="text-lg font-semibold text-gray-800 bg-[#F5F5DC] px-4 py-2 rounded-lg border-2 border-[#8B0000]">
